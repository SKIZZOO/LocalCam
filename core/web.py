...
_OLD_RECORDING_RE = re.compile(
    r'(?P<camera>.+)_(?P<stamp>\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2})_(?P<reason>Continuous|Motion|Manual)\.mkv(root: Path, rel: str) -> Path | None:
    try:
        base = root.resolve()
        target = (base / urllib.parse.unquote(rel.lstrip('/'))).resolve()
        target.relative_to(base)
...
def recording_meta(name: str, fallback_time: datetime | None = None) -> dict[str, Any]:
    match = _NEW_RECORDING_RE.match(name) or _OLD_RECORDING_RE.match(name)
    if match:
        stamp = match.group('stamp')
        fmt = '%Y-%m-%d-%H-%M-%S' if '-' in stamp else '%Y-%m-%d_%H-%M-%S'
        try:
            start = datetime.strptime(stamp, fmt)
        except ValueError:
            start = fallback_time or datetime.fromtimestamp(0)
        reason = match.group('reason').upper()
        camera = match.group('camera').replace('-', ' ') if match.re is _NEW_RECORDING_RE else match.group('camera')
